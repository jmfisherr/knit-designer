...
export default async function handler(req, res) {
  ...
  const userIdentifier = session ? sanitizeName(session.user.email || session.user.name || session.user.sub) : null
  const userFile = userIdentifier ? path.join(DATA_DIR, userIdentifier, id + '.json') : null
  const rootFile = path.join(DATA_DIR, id + '.json')
  ...
  if (req.method === 'PUT') {
    try {
      ...
      const newStem = safe ? `${uniq}-${safe}` : id
      ...
      const oldFile = path.join(userDir, id + '.json')
